...
class OrderPlaceView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        conn = StrictRedis(host="192.168.12.232", port="6379", db=2)
...
class OrderCommitView1(View):
    ...
    def post(self, request):
        ...
        if pay_method not in OrderInfo.PAY_METHODS.keys():
            return JsonResponse({"res": 2, "error": "非法的支付方式"})
        ...
        try:
            ...
            # 地址不存在
            return JsonResponse({"res": 3, "error": "地址非法"})
        ...
        try:
            ...
            # 用户的订单中有几个商品，需要向df_order_info表中添加几条记录
            from redis import StrictRedis
            conn = StrictRedis(host="192.168.12.232", port="6379", db=2)
            cart_key = 'cart_%d' % user.id
        ...
        # 返回应答
        return JsonResponse({"res": 5, "error": "创建成功"})
                ...
                for i in range(3):
                    ...
                    if res==0:
                        if i==2:
                            # 尝试3次
                            transaction.savepoint_rollback(save_id)
                            return JsonResponse({"res":7,"error":"下单失败2"})
                        continue
...
class OrderPayView(View):
    ...
    def post(self,request):
        ...
        user=request.user
        if not user.is_authenticated():
            return JsonResponse({"res":0,"error":"用户未登录"})
        ...
        # 校验参数
        if not order_id:
            return JsonResponse({"res":1,"error":"无效的订单id"})
    ...
    def post(self,request):
        ...
        while True:
            ...
            if code == "10000" and response.get("trade_status")=="TRADE_SUCCESS":
                ...
                order.save()
                # 返回结果
                return JsonResponse({"res":3,"error":"支付成功"})
                ...
                # 支付出错
                return JsonResponse({"res":4,"reeor":"支付失败"})
